...
def load_special_cases_dataset(dataset_path: Path):
    # Load json dataset
    for filesname in dataset_path.iterdir():
        if filesname.suffix == ".json" and "dataset_special_cases" in filesname.stem:
...
def generate_special_cases(llm: LLM, dataset_path: Path) -> list[str]:
    ds = load_special_cases_dataset(dataset_path)
                ...
                "content": [
                    {
                        ...
                        "image_url": {
                            "url": get_asset(
                                Path("raw_documents/dataset_special_cases")
                                / row["file"]
                            ),
                        },
                    },
