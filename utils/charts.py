...
def load_charts_dataset(dataset_path: Path):
    # Load json dataset
    for filesname in dataset_path.iterdir():
        if filesname.suffix == ".json" and "dataset_charts" in filesname.stem:
...
def generate_charts(llm: LLM, dataset_path: Path) -> list[str]:
    ds = load_charts_dataset(dataset_path)
                ...
                "content": [
                    {
                        ...
                        "image_url": {
                            "url": get_asset(
                                Path("raw_documents/dataset_charts") / row["file"]
                            ),
                        },
                    },
                    {
